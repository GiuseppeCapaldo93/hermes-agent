...
class TestIterFallbackEntriesStrings:
    ...
    def test_simple_provider_model_string(self):
        assert _iter_fallback_entries(["openrouter:anthropic/claude-3.5"]) == [
            {"provider": "openrouter", "model": "anthropic/claude-3.5"}
        ]

    def test_qualified_custom_provider_keeps_full_provider(self):
        # The provider itself contains a colon -> split on the LAST colon so the
        # qualified custom endpoint id survives intact.
        assert _iter_fallback_entries(["custom:my-endpoint:claude-sonnet-4.6"]) == [
            {"provider": "custom:my-endpoint", "model": "claude-sonnet-4.6"}
        ]

    def test_whitespace_is_trimmed(self):
        assert _iter_fallback_entries([" openai : gpt-5.2 "]) == [
    ...
    def test_bare_model_without_provider_is_skipped(self):
        ...
        assert _iter_fallback_entries(["gpt-5.2"]) == []

    def test_top_level_string(self):
        assert _iter_fallback_entries("custom:m1") == [
            {"provider": "custom", "model": "m1"}
        ]
